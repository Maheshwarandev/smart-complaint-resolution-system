...
const validateEmail = (email) => {
  ...
  if (email.length > 254) throw new AppError('Email is too long', 400);
  return email;
};
...
const validateName = (name) => {
  ...
  if (name.length > 100) throw new AppError('Name is too long', 400);
  if (!/^[a-zA-Z\s\-']+$/.test(name)) throw new AppError('Name contains invalid characters', 400);
  return name;
};
...
const validateComplaintTitle = (title) => {
  if (!title || typeof title !== 'string') throw new AppError('Complaint title is required', 400);
  title = title.trim();
  if (title.length < 5) throw new AppError('Complaint title must be at least 5 characters', 400);
  if (title.length > 200) throw new AppError('Complaint title is too long (max 200 characters)', 400);
  return title;
};

const validateComplaintDescription = (description) => {
  if (!description || typeof description !== 'string') throw new AppError('Complaint description is required', 400);
  description = description.trim();
  if (description.length < 10) throw new AppError('Complaint description must be at least 10 characters', 400);
  if (description.length > 2000) throw new AppError('Complaint description is too long (max 2000 characters)', 400);
  return description;
};

const validateCategory = (category) => {
  if (!category || typeof category !== 'string') throw new AppError('Category is required', 400);
...
const validatePriority = (priority) => {
  if (!priority || typeof priority !== 'string') throw new AppError('Priority is required', 400);
